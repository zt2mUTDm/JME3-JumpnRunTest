...
public final class SetMaxDistanceCommand implements Command {
	private final BiConverter<String, Spatial, Spatial> spatialTarget;
	
	public SetMaxDistanceCommand(final BiConverter<String, Spatial, Spatial> spatialTarget) {
	...
	public void execute(final Spatial caller, final String[] cmd, final Runnable done) {
		...
		if(spatial instanceof AudioNode) {
			((AudioNode) spatial).setMaxDistance(Float.parseFloat(cmd[2]));
		} else {
