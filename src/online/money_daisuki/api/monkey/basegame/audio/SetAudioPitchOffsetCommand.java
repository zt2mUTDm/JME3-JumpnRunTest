...
public final class SetAudioPitchOffsetCommand implements Command {
	private final BiConverter<String, Spatial, Spatial> spatialTarget;
	
	public SetAudioPitchOffsetCommand(final BiConverter<String, Spatial, Spatial> spatialTarget) {
	...
	public void execute(final Spatial caller, final String[] cmd, final Runnable done) {
		...
		if(spatial instanceof AudioNode) {
			((AudioNode) spatial).setPitch(Float.parseFloat(cmd[2]));
		} else {
