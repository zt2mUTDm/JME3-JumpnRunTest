...
public final class SetAudioTimeOffsetCommand implements Command {
	private final BiConverter<String, Spatial, Spatial> spatialTarget;
	
	public SetAudioTimeOffsetCommand(final BiConverter<String, Spatial, Spatial> spatialTarget) {
	...
	public void execute(final Spatial caller, final String[] cmd, final Runnable done) {
		...
		if(spatial instanceof AudioNode) {
			((AudioNode) spatial).setTimeOffset(Float.parseFloat(cmd[2]));
		} else {
