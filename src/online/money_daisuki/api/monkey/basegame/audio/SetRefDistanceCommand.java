...
public final class SetRefDistanceCommand implements Command {
	private final BiConverter<String, Spatial, Spatial> spatialTarget;
	
	public SetRefDistanceCommand(final BiConverter<String, Spatial, Spatial> spatialTarget) {
	...
	public void execute(final Spatial caller, final String[] cmd, final Runnable done) {
		...
		if(spatial instanceof AudioNode) {
			((AudioNode) spatial).setRefDistance(Float.parseFloat(cmd[2]));
		} else {
