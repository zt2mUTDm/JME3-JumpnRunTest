...
public final class AddPlayerCommand implements Command {
	...
	public AddPlayerCommand(final BiConverter<? super String, ? super Spatial, ? extends Node> nodeTarget,
			final DataSink<Spatial> playerContainer) {
	...
	public void execute(final Spatial caller, final String[] cmd, final Runnable done, final SimpleApplication app) {
		...
		final Form form = app.getStateManager().getState(FormLoadAppState.class).load(cmd[1]);
		final PhysicsObject po = form.getPhysicsObject();
		final Spatial spatial = po.getSpatial();
		
		if(!app.getStateManager().getState(PythonAppState.class).addScript(form)) {
			done.run();
			return;
		}
