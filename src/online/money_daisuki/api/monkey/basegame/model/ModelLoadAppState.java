...
public final class ModelLoadAppState extends BaseAppState {
	...
	private Spatial meshToGeometry(final JsonMap map, final Mesh mesh) {
		final MaterialLoadAppState matLoader = app.getStateManager().getState(MaterialLoadAppState.class);
		
		final Material mat = matLoader.loadMaterial(map.get("material").asData().asString());
