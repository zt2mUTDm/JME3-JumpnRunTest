...
public final class ModelLoader implements AssetLoader {
	@Override
	public Spatial load(final AssetInfo assetInfo) throws IOException {
		try(final Reader in = new InputStreamReader(assetInfo.openStream())) {
			final JsonMap main = new JsonDecoder(in).decode().asMap();
			return(loadSpatial(main, assetInfo));
		}
	}
	private Spatial loadSpatial(final JsonMap map, final AssetInfo assetInfo) {
		final Spatial spatial = loadSpatialType(map, assetInfo);
	...
	private Spatial loadSpatialType(final JsonMap map, final AssetInfo assetInfo) {
		final String type = map.get("type").asData().asString();
		switch(type) {
			case("node"):
				return(loadNode(map, assetInfo));
			case("audio"):
				return(loadAudioNode(map, assetInfo));
			case("model"):
				return(loadModel(map, assetInfo));
			case("json"):
				return(loadJson(map, assetInfo));
			case("box"):
				return(loadBox(map, assetInfo));
			case("stripBox"):
				return(loadStripBox(map, assetInfo));
			case("cylinder"):
				return(loadCylinder(map, assetInfo));
			case("sphere"):
				return(loadSphere(map, assetInfo));
			case("dome"):
				return(loadDome(map, assetInfo));
			default:
				throw new IllegalStateException("Unreconizable type: " + type);
		}
	}
	private Node loadNode(final JsonMap map, final AssetInfo assetInfo) {
		final Node node = new Node();
		
		if(map.containsKey("childs")) {
			final JsonList children = map.get("childs").asList();
			for(final JsonElement e:children) {
				node.attachChild(loadSpatial(e.asMap(), assetInfo));
			}
		}
		return(node);
	}
	private Spatial loadAudioNode(final JsonMap map, final AssetInfo assetInfo) {
		final JsonMap e = map.get("audio").asMap();
		
		final String url = e.get("url").asData().asString();
		final DataType dType = Requires.notNull(DataType.valueOf(e.get("dataType").asData().asString()));
		
		final AudioNode audio = new AudioNode(assetInfo.getManager(), url, dType);
		audio.setDirectional(e.get("directional").asData().asBool());
		audio.setPositional(e.get("positional").asData().asBool());
		
		if(e.containsKey("direction")) {
			final JsonList direction = e.get("direction").asList();
	...
	private Spatial loadModel(final JsonMap map, final AssetInfo assetInfo) {
		final String url = map.get("url").asData().asString();
		return(assetInfo.getManager().loadModel(url));
	}
	private Spatial loadJson(final JsonMap map, final AssetInfo assetInfo) {
		final String url = map.get("url").asData().asString();
		return(assetInfo.getManager().loadModel(url));
	}
	
	private Spatial loadBox(final JsonMap map, final AssetInfo assetInfo) {
		...
		return(meshToGeometry(map, mesh, assetInfo));
	}
	private Spatial loadStripBox(final JsonMap map, final AssetInfo assetInfo) {
		final JsonList e = map.get("extends").asList();
	...
	private Spatial loadCylinder(final JsonMap map, final AssetInfo assetInfo) {
	...
	private Spatial loadSphere(final JsonMap map, final AssetInfo assetInfo) {
	...
	private Spatial loadDome(final JsonMap map, final AssetInfo assetInfo) {
	...
	private Spatial meshToGeometry(final JsonMap map, final Mesh mesh, final AssetInfo assetInfo) {
		final Material mat = assetInfo.getManager().loadMaterial(map.get("material").asData().asString());
